...
def dataset_body():
    ...
    st.markdown(f'''<p style="
                    ...
                    background-color:rgba(246, 245, 242, .2);
                    border-radius:5px;
                    padding:10px;">The dataset was provided by the Mexican government and is sourced from Kaggle. 
    ...
    st.write(f"""<p style="font-size:20px;color:white;">Size of the dataset: <span style='font-weight:bold;font-size:22px;'>{df.shape} </span></p>""",unsafe_allow_html=True)
    ...
    st.write(f'''<p style="color:white;">
    We see that all males have value 97 for pregnancy because it is missing. Given that males cannot get pregnant, 
    we will change all 97's in PREGNANT to 2. Some females have 98 which is a missing value, so we change all the 
    98's in PREGNANT to NAN. Currently 1 represents True and 2 represents False. To change the boolean values to 
    the conventional way, we change the 2's to 0's.</p>''',unsafe_allow_html=True)
...
df.loc[:, 'PREGNANT'] = df.loc[:, 'PREGNANT'].replace(2, 0)
    ...
    st.write(f'''<p style="color:white;">Some columns contain 3-4 unique values when it should be a boolean: YES or NO. We replace all 97's, 
                98's, and 99's to NAN because they are missing values. The 1's mean YES and 2's mean NO, so we 
                change 2's to 0's, the conventional way to represent Boolean values.</p>''',unsafe_allow_html=True)
...
df.loc[:, var] = df.loc[:, var].replace(2, 0)
    ...
    st.write(f'''<p style="color:white;">DATE_DIED is represent as a pandas date format of when the patient died. If we have 9999-99-99 values, that means this patient is alive. 
                 ...
                 if the patient is alive or 1 if the patient died.</p>''',unsafe_allow_html=True)
...
df.drop(columns='DATE_DIED', inplace=True)
    ...
    st.write(f'''<p style="color:white;">To extract the features that has the lowest correlation with the column DIED, 
                 we made a correlation test.</p>''',unsafe_allow_html=True)
...
lowest_4_features = sorted_correlation.head(4).index      
    ...
    st.write(f"""<p style="color:white;">According to correlation test, we found out these 4 features with 
                lowest correlation with 'DIED' column""",unsafe_allow_html=True)
...
Index(['TOBACCO', 'ASTHMA', 'PREGNANT', 'INMSUPR'], dtype='object')  
    ...
    st.write(f"""<p style="color:white;">Thus, we will drop the features that have low correlation with DIED feature.</p>""",unsafe_allow_html=True)
