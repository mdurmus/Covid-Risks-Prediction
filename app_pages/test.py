...
def test_body():
    ...
    st.markdown('''
                ...
                <h3>F1-Score</h3>
                <p style="color:white;">
                The F1 Score is a metric used in binary classification tasks, combining both precision and recall into a single measure. It's calculated as the harmonic mean of precision and recall. Here's the formula:</p>
    ...
    st.markdown('''
                ...
                <p style="color:white;">ROC (Receiver Operating Characteristic) Curve is a graphical representation of the performance of a binary classification model, such as a decision tree classifier, across different threshold values. However, decision trees are not typically evaluated using ROC curves, as ROC curves are more commonly associated with models that output probabilities (e.g., logistic regression or support vector machines).</p>
